...
export class AddQuestionsComponent implements OnInit {
    ...
    applyFormat(command: string) {
        ...
        if (!selected) {
            this.toast.error('Please select some text first!');
            return;
        }
        ...
        const [open, close] = formatMap[command];
    ...
    renderQuestion(text: string): string {
        ...
        return text
            ...
            .replace(/%%U%%(.*?)%%\/U%%/g, '<u>$1</u>')
            .replace(/%%C%%(.*?)%%\/C%%/g, '<code class="bg-gray-100 px-1 rounded text-xs font-mono">$1</code>')
